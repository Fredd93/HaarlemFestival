...
if ($event->type === 'checkout.session.completed') {
    ...
    if ($paymentIntent && isset($paymentIntent->charges->data[0])) {
        ...
        try {
            $orderRes = json_decode(file_get_contents("https://retailers-ericsson-wax-married.trycloudflare.com/api/orders/create", false, stream_context_create([
                'http' => [
                    'method'  => 'POST',
            ...
            $invoiceRes = json_decode(file_get_contents("https://retailers-ericsson-wax-married.trycloudflare.com/api/invoices", false, stream_context_create([
                'http' => [
                    'method'  => 'POST',
                    'header'  => "Content-Type: application/json\r\n",
                    'content' => json_encode([
